...
class JWTRepo:
    ...
    def generate_token(self, expires_delta: Optional[timedelta] = None):
        ...
        encode_jwt = jwt.encode(to_encode, get_settings().SECRET_KEY, algorithm= get_settings().ALGORITHM)
    ...
    def decode_token(self):
        try:
            decode_token = jwt.decode(
                self.token, get_settings().SECRET_KEY, algorithms=[get_settings().ALGORITHM])
...
class JWTBearer(HTTPBearer):
    ...
    def verify_jwt(jwt_token: str):
        return True if jwt.decode(jwt_token, get_settings().SECRET_KEY, algorithms=[get_settings().ALGORITHM]) is not None else False
